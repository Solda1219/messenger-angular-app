...
export class LearnRecordComponent implements OnInit {
 loading;
  item = [];
  tbNewRecordHeader = ['Title', 'Content','Sender','Receiver','R/S','MessageTime','RecordingSize','CloudCopied','PlaybackCount'];
  tbNewRecordCol = ['title', 'content','sender','receiver','received','created_at','cloudCopy','fileSize','is_read'];
  ...
  displayedColumnsNewRecording: string[] = ['title', 'sender','receiver', 'received','created_at','cloudCopy','fileSize','is_read','Action'];
  ...
  //recording new
  createRocordingModalShow() {
    $('#createRocordingModal').modal('show')
  }
  createRocordingModalClose() {
    $('#createRocordingModal').modal('hide')
  }
  async recordingNew() {
    this.loading = true;
    try {
      const res = await this.userService.postRequest('_api/learn/learnRecord/recordingnew').toPromise();
  ...
  //old part have to think to delete
  async search() {
