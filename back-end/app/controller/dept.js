...
let create = async (req, res) => {
  try {
    const data = req.body;
    const exist_id = await model.existID(data.dept_id);
    if(exist_id){
      data.updated_at = core_func.strftime(Date.now())
      const exist = await model.existNotMe(data.dept_id, data.dept_name);
...
let del = async (req, res) => {
  try {
    await model.del(req.body.dept_id,req.body.dept_level);
    return res.json({message: 'Success' });
  }
  catch (error) {
    return res.status(400).json({
